...
class Authenticator
{
    ...
    public function createSecret($secretLength = 16)
    {
        ...
        if (function_exists('random_bytes')) {
            ...
            $rnd = random_bytes($secretLength, MCRYPT_DEV_URANDOM);
    ...
    public function getQRCodeUrl($name, $secret, $title = null, $params = [])
    {
        ...
        $otpUrl = "otpauth://totp/" . rawurlencode($name) . "?secret=" . rawurlencode($secret);
        if (!empty($title)) {
            $otpUrl .= "&issuer=" . rawurlencode($title);
        }

        // QR Code API URL
        return "https://api.qrserver.com/v1/create-qr-code/?data=" . urlencode($otpUrl) . "&size={$width}x{$height}&ecc=$level";
    }
    ...
    public function setCodeLength($length): Authenticator
    {
